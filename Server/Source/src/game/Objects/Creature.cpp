...
void Creature::SelectLevel(CreatureInfo const* cinfo, float percentHealth, float percentMana)
{
	...
	//-----------------------
	uint32 updatedMinLevel = minlevel;
	uint32 updatedMaxLevel = maxlevel;

	if (level > 1) {
		level *= 6;
		updatedMinLevel = minlevel * 6;
		updatedMaxLevel = maxlevel * 6;
	}
	else {
		level *= 2;
		updatedMinLevel = minlevel * 2;
		updatedMaxLevel = maxlevel * 2;
	}

	float rellevel = updatedMaxLevel == updatedMinLevel ? 0 : (float(level - updatedMinLevel)) / (updatedMaxLevel - updatedMinLevel);
	//-----------------------
...
float Creature::GetAttackDistance(Unit const* pl) const
{
	...
	//L10 MODIFIED
	uint32 const playerlevel = pl->GetLevelForTarget(this) * 6;
	uint32 const creaturelevel = GetLevelForTarget(pl) * 6;
	//-----------
