...
SpellMissInfo WorldObject::MagicSpellHitResult(Unit* pVictim, SpellEntry const* spell, Spell* spellPtr)
{
	...
	//L10 MODIFIED
	//Adding spell school skill value resist chance
	if (this->IsPlayer()) {
		uint16 value = this->ToPlayer()->GetSkillValueForSchool(spell->School);
		uint32 level = this->ToPlayer()->GetLevel();
		if (value > level * 5) {
			missChance -= (value - (level * 5)) * 100;
		}
	}
	//----------
...
uint32 WorldObject::CalcArmorReducedDamage(Unit* pVictim, uint32 const damage) const
{
	...
	//L10 MODIFIED
	//Because we have a lvl10 server, we must multiply getLevel() by 6 to get the classic values
	float levelModifier = (float)GetLevel() * 6;
...
int32 WorldObject::CalculateSpellDamage(Unit const* target, SpellEntry const* spellProto, SpellEffectIndex effect_index, int32 const* effBasePoints, Spell* spell)
{
	...
	if (spellProto->Attributes & SPELL_ATTR_LEVEL_DAMAGE_CALCULATION && spellProto->spellLevel &&
		...
		bool levelNeedsIncreasing = false;
		if (target != nullptr) {
			if (pUnit != nullptr) {
				if (target->IsPlayer() && pUnit->IsCreature()) {
					levelNeedsIncreasing = true;
				}
			}
		}
		if (levelNeedsIncreasing) {
			value = int32(value * 0.25f * exp((GetLevel() * 6) * (70 - (spellProto->spellLevel * 6)) / 1000.0f));
		}
		else {
