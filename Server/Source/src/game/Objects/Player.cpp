...
//L10 MODIFIED -----------------------------------------
//These spells(ids) will give spell skill points
static std::vector<uint32> spellsToAcceptSkilling = {
	0
};

//These spells(ids) won't give spell skill points
static std::vector<uint32> spellsToRejectSkilling = {
	//Paladin spells:
	//Blessing of Wisdom---------------------------
	19742,19850,19852,19853,19854,25290,25894,25918,
	//Priest spells:
	//Blessed Recovery,Holy Nova-------------------------,
	27813,27817,27818,//15237,15430,15431,27799,27800,27801,
	//Renew----------------------------------------------,
	139,6074,6075,6076,6077,6078,10927,10928,10929,25315,
	//Psychic Scream-----------,Abolishes,Dispels,MindVision,
	8122,8124,10888,10890,27610,552,1268,527,988,2096,10909,
	//Mage spells:
	//Arcane Explosion-------------,Blast Wave-------------------,Flamestrike--------------------,
	//1449,8437,8438,8439,10201,10202,11113,13018,13019,13020,13021,2120,2121,8422,8423,10215,10216,
	//Blizzard--------------------,Cone of Cold--------------,Frost Nova--------,Frostb,Chilled effect
	10,6141,8427,10185,10186,10187,//120,8492,10159,10160,10161,122,865,6131,10230,12494,
	//Warlock spells:
	//Hellfire Effect,
	5857,11681,11682,
	//Shaman spells:
	//Flametongue Attack---,Lightning Shield------------------------,
	29469,29470,10444,16368,26364,26545,325,905,945,8134,10431,10432,
	//Druid spells:
	//Moonfire---------------------------------------,Starfire---------------------,Tranquility-------,
	8921,8924,8925,8926,8927,8928,8929,9833,9834,9835,2912,8949,8950,8951,9875,9876,740,8918,9862,9863,
	//Abolish,Hurricane--------,Rejuveration------------------------------------,
	2893,3137,16914,17401,17402,774,1058,1430,2090,2091,3627,8910,9839,9840,9841,
	//Hunter spells:
	//Arcane shot---------------------------------,Conc,
	3044,14281,14282,14283,14284,14285,14286,14287,5116
};

static std::vector<uint32> spellsThatShouldNotFail = {
	//Frost/Ice Armor Chilled effects
	6136,7321,
	//Flametongue weapon effect
	8025,8031,8032,10446,16347,16348,8026,8028,8029,10445,16343,16344,29469,29470,10444,
	//Frostbrand weapon effect
	8034,8037,10458,16352,16353,8035,8039,10457,16357,16358,
	//Windfury weapon effect
	8233,8234,8236,8237,10484,10488,16361,16363
};
//----------------------------------------------------
...
//L10 MODIFIED
void Player::PrintXpLimitReachedToChat() {
	std::string className = GetClassNameById(GetClass());
	uint32 nextLevel = GetLevel() + 1;
	std::string text = "You have gathered enough experience to take the next step on the path of the " + className + ". Visit your trainer and prove your worth so that you can reach level " + std::to_string(nextLevel) + ".";
	char str[250];
	ChatHandler(this).SendSysMessage(strncpy(str, text.c_str(), sizeof(str)));
}

//L10 MODIFIED
//Whole method is new
std::string Player::GetClassNameById(uint8 classId) {
	switch (classId)
	{
	case 1:
		return "Warrior";
...
uint32 Player::EnvironmentalDamage(EnvironmentalDamageType type, uint32 damage)
{
	...
	//L10 MODIFIED
	//If we are undead, slime should not affect us
	if (type == DAMAGE_SLIME && GetRace() == RACE_UNDEAD) {
		absorb = damage;
		resist = 0;
	}
    ...
    if (m_MirrorTimerFlags & UNDERWATER_INWATER)
    {
        ...
        else
        {
            ...
            if (m_MirrorTimer[BREATH_TIMER] < 0)
            {
				...
				//L10 MODIFIED
				uint32 damage = GetMaxHealth() / 5 + urand(0, (GetLevel() * 6) - 1);
...
void Player::AddToWorld()
{
	...
	//L10 MODIFIED
	//Login message if xp is at its limit
	uint32 curXP = GetUInt32Value(PLAYER_XP);
	uint32 nextLvlXP = GetUInt32Value(PLAYER_NEXT_LEVEL_XP);
	if (curXP == nextLvlXP - 1) {
		PrintXpLimitReachedToChat();
	}
}
...
void Player::RewardRage(uint32 damage, bool attacker)
{
	...
	//L10 MODIFIED
	//target rageconversion at level 10: 0.16232598701107614
	//target rageconversion at level 1: 1
	//rageconversion step value: 0.0930748903321027
	float l10value = 1;
	if (GetLevel() > 1 && GetLevel() < 10) {
		l10value = (1 - ((GetLevel() - 1) * 0.0930748903321027));
	}
	else if (GetLevel() == 10) {
		l10value = 0.16232598701107614;
	}
	addRage *= l10value;
...
//L10 MODIFIED
//Most of the method is modified - extended with level up quest codes
void Player::GiveXP(uint32 xp, Unit* victim, bool allowLeveling)
{
	if (xp < 1)
		return;
	...
	//WORKAROUND - ONLY UNTIL LUP QUESTS ARE FINISHED
	allowLeveling = true;
	//---------
	...
	if (!allowLeveling) {
		...
		if (newXP >= nextLvlXP) {
			newXP = nextLvlXP - 1;
			//There are 9 quests, starting at 51050
			CompleteQuest(51049 + level);
			PrintXpLimitReachedToChat();
		}
	}
...
uint32 Player::GetResetTalentsCost() const
{
    ...
    {
		//L10 MODIFIED - GOLD -> SILVER
		return sWorld.getConfig(CONFIG_UINT32_RESPEC_BASE_COST) * SILVER;
    }
    ...
    {
		//L10 MODIFIED - GOLD -> SILVER
		return (multiCost * maxMulti) * SILVER;
    }

	//L10 MODIFIED - GOLD -> SILVER
	return (m_resetTalentsMultiplier * multiCost) * SILVER;
}
...
float Player::GetMeleeCritFromAgility() const
{
	...
	//L10 MODIFIED
	uint32 level = GetLevel() * 6;
	float classrate = valLevel1 * float(60.0f - level) / 59.0f + valLevel60 * float(level - 1.0f) / 59.0f;
	//ORIGINAL
...
// Chance to crit is computed from INT and LEVEL as follows:
    ...
    if (GetTypeId() == TYPEID_PLAYER)
    {
		...
		//L10 MODIFIED
		float crit_ratio = crit_data[my_class].rate0 + crit_data[my_class].rate1 * (GetLevel() * 6);
		//ORIGINAL
...
//L10 MODIFIED
//The whole method is modded!
void Player::UpdateCombatSkills(Unit* pVictim, WeaponAttackType attType, bool defence)
{
	if (!pVictim) {
		return;
	}
	// if defense than pVictim == attacker
	uint32 plevel = GetLevel();
	uint32 moblevel = pVictim->GetLevelForTarget(this);

	if (plevel > 1 && moblevel <= plevel - 2) {
		return;
	}
	//No skilling when attacking training dummies
	if (pVictim->GetEntry() != 0) {
		if (pVictim->GetEntry() == 1000002 || pVictim->GetEntry() == 1000003 || pVictim->GetEntry() == 1000004) {
			return;
		}
	}
	//Lets just check the chance in the config file
	float def_chance = sWorld.getConfig(CONFIG_UINT32_SKILL_GAIN_DEFENSE);
	float wpn_chance = sWorld.getConfig(CONFIG_UINT32_SKILL_GAIN_WEAPON);

	float chance = 1.0f;
	if (defence) chance = def_chance;
	else chance = wpn_chance;

	//level settings
	if (moblevel == plevel - 1) chance *= 0.5f;
	else if (moblevel == plevel) chance *= 1.0f;
	else if (moblevel == plevel + 1) chance *= 1.5f;
	else if (moblevel == plevel + 2) chance *= 2.0f;
	else chance *= 2.5f;

	chance += 0.1f * GetStat(STAT_INTELLECT);

	//minimum chance to increase skill is 1%, and the maximum is 100%
	chance = chance < 1.0f ? 1.0f : chance;
	chance = chance > 100.0f ? 100.0f : chance;
...
//L10 MODIFIED
//Whole method is new
void Player::UpdateSpellSkill(const SpellEntry* spell, const Unit* target, int targetCount) {
	if (spell == nullptr) return;

	//No skilling when attacking training dummies... the problem is that we don't always have a victim
	Unit* victim = GetVictim();
	if (victim) {
		if (victim->GetCreatureType() == CREATURE_TYPE_CRITTER) {
			//No skilling on critters.
			return;
		}
		if (victim->GetEntry() != 0) {
			if (victim->GetEntry() == 1000002 || victim->GetEntry() == 1000003 || victim->GetEntry() == 1000004) {
				return;
			}
		}
	}
	if (spell->IsHealSpell()) {
		//Healing skilling rules -> not finished
		if (target) {
	...
	//There are a few aoe spells which isnt considered aoe for some reason, like frost nova
	if (spell->IsAreaOfEffectSpell()) {
		//TODO: AoE skilling rules -> not finished: Some aoe spells like Blizzard and Rain of Fire doesn't produce targets and thus 0 targetCount
		if (targetCount == 0) {
			return;
		}
	}
	...
	if (spellLevel < plevel - 2) chance *= 0.1f;
	else if (spellLevel == plevel - 2) chance *= 0.33f;
	else if (spellLevel == plevel - 1) chance *= 0.66f;
	else if (spellLevel == plevel) chance *= 1.0f;
	else if (spellLevel > plevel) chance *= 1.5f; //rare case, but could be possible

	chance += 0.1f * (GetStat(STAT_INTELLECT) / (float)plevel);
	//---------------------------

	//TODO: Here we need reduced chance if players are using lower ranks of spells

	if (spell->School == 0 || spell->School < 0) return;
	//2 = fire, 4 = frost, 6 = arcane, 1 = holy, 5 = shadow, 3 = nature
	if (roll_chance_f(chance)) {
		switch (spell->School)
		{
		case 2:
			UpdateSkill(SKILL_SPELL_FIRE, 1);
			break;
		case 4:
			UpdateSkill(SKILL_SPELL_FROST, 1);
			break;
		case 6:
			UpdateSkill(SKILL_SPELL_ARCANE, 1);
			break;
		case 1:
			UpdateSkill(SKILL_SPELL_HOLY, 1);
			break;
		case 5:
			UpdateSkill(SKILL_SPELL_SHADOW, 1);
			break;
		case 3:
			UpdateSkill(SKILL_SPELL_NATURE, 1);
			break;
		default:
			break;
		}
...
//L10 MODIFIED
//Whole method is new
//TRUE means REJECTED - FALSE means NOT REJECTED
bool Player::IsSpellRejected(const SpellEntry* spell) {
	//If there'll be spells that we can't exclude otherwise, fill a std::vector<uint32> and check its content
	//Or if we want healing spells to increase skill, put it here. But looks like we can't get the target from here.
	//Spells to reject by ID
	if (!spell) {
		sLog.outInfo("No spell to reject");
		return true;
	}

	if (std::find(spellsToAcceptSkilling.begin(), spellsToAcceptSkilling.end(), spell->Id) != spellsToAcceptSkilling.end()) {
		return false;
	}
	//Spells to reject by ID
	if (std::find(spellsToRejectSkilling.begin(), spellsToRejectSkilling.end(), spell->Id) != spellsToRejectSkilling.end()) {
		return true;
	}

	if (!CheckClassSchools(spell->School)) {
		return true;
	}
	uint16 skillValue;
	skillValue = GetSkillValueForSchool(spell->School);
	if (skillValue == 0) {
		return true;
	}
	//If it's not a damaging spell, return
	if (spell->IsDirectDamageSpell()) return false;
	if (spell->IsSpellAppliesPeriodicAura()) return false;
	if (spell->IsCCSpell()) return false;
	if (spell->IsChanneledSpell()) return false;
	if (spell->IsCharmSpell()) return false;
	if (spell->IsHealSpell()) return false;
	if (spell->IsAreaOfEffectSpell()) return false;
	//TEMPORARILY, until we develop a system which checks totem spell casts
	if (spell->IsTotemSummonSpell()) return false;
...
//L10 MODIFIED
//Whole method is new
bool Player::CheckClassSchools(uint32 spellSchool) {
	//If the player can't increase a spell skill for the casted spell
	//because its an item or rare exception, it should never fail
	...
	if (pClass != 0) {
		...
		case 2:
			//Pala
			if (spellSchool != 1) return false;
			break;
		case 5:
			//Priest
			if (spellSchool != 1 && spellSchool != 5) return false;
			break;
		case 7:
			//Shaman
			if (spellSchool != 2 && spellSchool != 3 && spellSchool != 4) return false;
			break;
		case 8:
			//Mage
			if (spellSchool != 2 && spellSchool != 4 && spellSchool != 6) return false;
			break;
		case 9:
			//Warlock
			if (spellSchool != 2 && spellSchool != 5) return false;
			break;
		case 11:
			//Druid
			if (spellSchool != 3) return false;
			break;
		default:
			//Not one of the classes with spell skills, return
			return false;
		}
	}
	else {
		sLog.outInfo("Class value not found!");
	}
	return true;
}
...
//L10 MODIFIED
//Whole method is new
uint16 Player::GetSkillValueForSchool(uint32 school) {
	uint16 skillValue;

	switch (school)
	{
	case 2:
		skillValue = GetSkillValue(SKILL_SPELL_FIRE);
		break;
	case 4:
		skillValue = GetSkillValue(SKILL_SPELL_FROST);
		break;
	case 6:
		skillValue = GetSkillValue(SKILL_SPELL_ARCANE);
		break;
	case 1:
		skillValue = GetSkillValue(SKILL_SPELL_HOLY);
		break;
	case 5:
		skillValue = GetSkillValue(SKILL_SPELL_SHADOW);
		break;
	case 3:
		skillValue = GetSkillValue(SKILL_SPELL_NATURE);
		break;
	default:
		skillValue = 0;
		break;
...
//L10 MODIFIED
//Whole method is new
uint16 Player::GetSkillValueForSchoolMask(SpellSchoolMask schoolMask) {
	uint16 skillValue;
...
//L10 MODIFIED
//Whole method is new
bool Player::ShouldSpellFail(const SpellEntry* spell) {
	if (!CheckClassSchools(spell->School)) {
		return false;
	}

	if (std::find(spellsThatShouldNotFail.begin(), spellsThatShouldNotFail.end(), spell->Id) != spellsThatShouldNotFail.end()) {
		return false;
	}
	if (spell->IsPassiveSpell() || spell->IsPassiveSpellStackableWithRanks()) {
		return false;
	}
	//SelfHarm spells should not fail or trigger skilling
	if (spell->Id > 35017 && spell->Id < 35078) {
		return false;
	}
	...
	uint32 plevel = GetLevel();
	//At max skill or above, never fail
	if (skillValue >= plevel * 5) {
		return false;
	}

	//If not max, calculate a chance
	else {
		float chance = 0.0f;
		chance += ((plevel * 5) - skillValue);
		if (chance > 50.0f) chance = 50.0f;
		if (roll_chance_f(chance)) {
			return true;
		}
	}
...
//L10 MODIFIED
//Whole method is new
uint32 Player::GetSelfHarmSpell(uint32 school) {
	//Spell school IDs : Holy: 1, Fire : 2, Nature : 3, Frost : 4, Shadow : 5, Arcane : 6
	switch (school)
	{
	case 1:
	{
		uint32 holySpells[10] = { 35028,35029,35030,35031,35032,35033,35034,35035,35036,35037 };
		return holySpells[GetLevel() - 1];
	}
	case 2:
	{
		uint32 fireSpells[10] = { 35018,35019,35020,35021,35022,35023,35024,35025,35026,35027 };
		return fireSpells[GetLevel() - 1];
	}
	case 3:
	{
		uint32 natureSpells[10] = { 35038,35039,35040,35041,35042,35043,35044,35045,35046,35047 };
		return natureSpells[GetLevel() - 1];
	}
	case 4:
	{
		uint32 frostSpells[10] = { 35048,35049,35050,35051,35052,35053,35054,35055,35056,35057 };
		return frostSpells[GetLevel() - 1];
	}
	case 5:
	{
		uint32 shadowSpells[10] = { 35058,35059,35060,35061,35062,35063,35064,35065,35066,35067 };
		return shadowSpells[GetLevel() - 1];
	}
	case 6:
	{
		uint32 arcaneSpells[10] = { 35068,35069,35070,35071,35072,35073,35074,35075,35076,35077 };
		return arcaneSpells[GetLevel() - 1];
	}
	}

	sLog.outError("Could not find the proper self harm spell!");
	return 0;
}
...
void Player::UpdateSkillsForLevel()
{
    ...
    for (SkillStatusMap::iterator itr = mSkillStatus.begin(); itr != mSkillStatus.end(); ++itr)
    {
		...
		//L10 MODIFIED
		//Addition to make poisons and lockpicking skill values 300 again
		if (pSkill->id == SKILL_POISONS || pSkill->id == SKILL_LOCKPICKING) {
			maxSkill = GetLevel() * 30;
		}
		//------------
...
#endif
							//L10 MODIFIED
							if (pSkill->id == SKILL_LOCKPICKING || pSkill->id == SKILL_POISONS) {
								SetSkill(uint16(pSkill->id), newSkillValue, GetLevel() * 30);
							}
							else {
    ...
    if (!(currFields & val))
    {
        ...
        else if (p->AreaLevel > 0)
        {
            ...
            else
            {
				...
				//L10 MODIFIED
				if (diff < -1)
					XP = uint32(sObjectMgr.GetBaseXP(GetLevel() + 1) * explorationRate);
				else if (diff > 1)
				{
					int32 exploration_percent = (100 - ((diff - 1) * 1));
...
void Player::RewardQuest(Quest const* pQuest, uint32 reward, WorldObject* questEnder, bool announce)
{
	...
	//L10 MODIFIED
	if (GetLevel() < sWorld.getConfig(CONFIG_UINT32_MAX_PLAYER_LEVEL))
		if (pQuest->HasQuestFlag(QUEST_FLAGS_LEVEL_UP)) GiveXP(XP, nullptr, true);
		else GiveXP(XP, nullptr, false);
		//-------------
...
uint32 Player::GetMinLevelForBattleGroundBracketId(BattleGroundBracketId bracket_id, BattleGroundTypeId bgTypeId)
{
	...
	ASSERT(bg);
	if (bgTypeId == BATTLEGROUND_AB)
		return 2 * bracket_id + bg->GetMinLevel() - 2;
	else
		return 2 * bracket_id + bg->GetMinLevel();
}
...
{
	//L10 MODIFIED
	//New reforged calculations - 50 points
	uint32 talentPointsForLevel = GetLevel() * 5;
	//Old reforged calculations - 51 points
	//uint32 talentPointsForLevel = (GetLevel() * 6) - 9;
	//ORIGINAL
