...
uint32 Unit::DealDamage(Unit* pVictim, uint32 damage, CleanDamage const* cleanDamage, DamageEffectType damagetype, SpellSchoolMask damageSchoolMask, SpellEntry const* spellProto, bool durabilityLoss, Spell* spell)
{
    ...
    else                                                    // if (health <= damage)
    {
        ...
        else                                                // victim is a player
        {
            ...
            {
				//L10 MODIFIED
				//Only take durab on hit taken from armor, no weapons
                EquipmentSlots slot = EquipmentSlots(urand(0, 9));
                ((Player*)pVictim)->DurabilityPointLossForEquipSlot(slot);
            }
        }

		//L10 MODIFIED
		//Moved a lot down, to around 1450
        /*if (IsPlayer())
        {
            ...
            {
				//L10 MODIFIED
				//Only take durab on hit taken from weapons, no armor
                EquipmentSlots slot = EquipmentSlots(urand(15, 17));
...
void Unit::CalculateMeleeDamage(Unit* pVictim, uint32 damage, CalcDamageInfo* damageInfo, WeaponAttackType attackType)
{
    ...
    if (damageInfo->attackType == RANGED_ATTACK)
    {
		//L10 MODIFIED
		if (IsPlayer()) {
			//Ranged durability calculation
			if (roll_chance_f(sWorld.getConfig(CONFIG_FLOAT_RATE_DURABILITY_LOSS_DAMAGE)))
			{
				EquipmentSlots slot = EquipmentSlots(17);
				((Player*)this)->DurabilityPointLossForEquipSlot(slot);
			}
		}
		//-----------
    ...
    switch (damageInfo->hitOutCome)
    {
        ...
        case MELEE_HIT_NORMAL:
        {
			...
			if (IsPlayer()) {
				...
				{
					EquipmentSlots slot;
					if (isOffhandDamage) slot = EquipmentSlots(16);
					else slot = EquipmentSlots(15);
					((Player*)this)->DurabilityPointLossForEquipSlot(slot);
				}
			}
			//-----------
...
void Unit::DealMeleeDamage(CalcDamageInfo* damageInfo, bool durabilityLoss)
{
    ...
    if (damageInfo->totalDamage && !IsPlayer() && !((Creature*)this)->GetCharmerOrOwnerGuid() && !pVictim->HasInArc(M_PI_F, this))
    {
		...
		//L10 MODIFIED
		//Lets just set it to 20% for now
...
void Unit::CalculateDamageAbsorbAndResist(WorldObject* pCaster, SpellSchoolMask schoolMask, DamageEffectType damagetype, uint32 const damage, uint32* absorb, int32* resist, SpellEntry const* spellProto, Spell* spell)
{
    ...
    if (canResist || (resistanceChance < 0))
    {
        ...
        *resist = int32(int64(damage) * multiplier);
		...
		//L10 MODIFIED
		//With higher than max spell skill, the damage resisted should be less
		//The checks are here because weapon spell damages doesnt have spell objects
		if (pCaster->IsPlayer()) {
			uint16 value = 0;
			if (spellProto) {
				//If it has spellProto, then it's a regular player spell
				if (spellProto->School) value = pCaster->ToPlayer()->GetSkillValueForSchool(spellProto->School);
			}
			else if (schoolMask) {
				//If it doesn't have spellProto, it means that it's an item's spell, like weapon damage
				value = pCaster->ToPlayer()->GetSkillValueForSchool(schoolMask);
			}
			//Calculation
			uint32 level = pCaster->ToPlayer()->GetLevel();
			if (value > level * 5) {
				//The conversion will fail, if not everything is float
				float floatValue = (float)value;
				float floatLevel = (float)level;
				float skillMultiplier = ((100 - ((floatValue - (floatLevel * 5)) * 5)) / 100);
				if (skillMultiplier < 0.25) skillMultiplier = 0.25;
				float skillMultiplied = *resist * skillMultiplier;
				*resist = (int)std::roundf(skillMultiplied);
			}
		}
		//--------------
