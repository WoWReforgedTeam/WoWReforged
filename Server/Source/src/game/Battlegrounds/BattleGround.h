...
enum BattleGroundBracketId                                  // bracketId for level ranges
{
	...
	BG_BRACKET_ID_LAST = 3,                       // so for start level 10 will be 10-19, 20-29, ...  all greater max bg level included in last bracket

	MAX_BATTLEGROUND_BRACKETS = 4                        // used as one from values, so in enum
};
