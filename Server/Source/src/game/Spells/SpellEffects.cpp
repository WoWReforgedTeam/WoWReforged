...
void Spell::EffectDummy(SpellEffectIndex eff_idx)
{
    ...
    switch (m_spellInfo->SpellFamilyName)
    {
        ...
        case SPELLFAMILY_SHAMAN:
        {
			...
			//L10 MODIFIED
			//TODO: NOT WORKING RN! Requires the 35149 to only trigger after a critical, and should be on just for a second.
			//TODO: Right now I don't think we have a method do reduce cooldown, only to rest it sadly
			/*switch (m_spellInfo->Id)
			{
				case 35149:                                 // Bloodlust
				{
					if (m_caster->GetTypeId() != TYPEID_PLAYER)
						return;
					...
					{
						SpellEntry const* spellInfo = sSpellMgr.GetSpellEntry(itr->first);
						uint32 delay = ((Player*)m_caster)->GetSpellCooldownDelay(spellInfo->Id);
						uint32 scaling = 30;
						//sLog.outInfo("Its inside 35149, and recoveryTime1: %u", delay);
						if (!delay)
							break;

						if (spellInfo->Id == 35148 && delay > scaling) {
							//sLog.outInfo("Cooldown - %u sec for 35148", scaling);
							((Player*)m_caster)->SendSpellCooldown(spellInfo->Id, delay - scaling, ((Player*)m_caster)->GetObjectGuid());
							break;
						}
						else if (spellInfo->Id == 35148 && delay <= scaling) {
							//sLog.outInfo("Cooldown 0 for 35148");
							((Player*)m_caster)->RemoveSpellCooldown((itr++)->first, true);
							break;
						}
						else
							++itr;
						sLog.outInfo("Its inside 35149, and recoveryTime2: %u", ((Player*)m_caster)->GetSpellCooldownDelay(spellInfo->Id));
					}
					sLog.outInfo("Its inside 35149, and done");
					return;
				}
				break;
			}*/
