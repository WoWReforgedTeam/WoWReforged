...
//L10 MODIFIED
static std::vector<uint32> critterEntries = {
	3300, 15475, 10716, 3835, 2110, 14421, 5741, 5743, 5739, 5742,
	6368, 620, 15066, 13338, 16779, 2442, 12299, 12297, 13016, 883,
	3444, 10582, 8963, 13017, 14892, 890, 9699, 1352, 13321, 4166,
	5951, 385, 6653, 10780, 10779, 15010, 10541, 15065, 16068, 9700,
	16030, 4953, 6271, 16998, 12383, 7208, 582, 9600, 15371, 7898,
	10461, 10536, 10441, 12120, 16369, 16373, 2620, 721, 2098, 4075,
	8881, 5076, 11776, 6145, 15476, 16372, 1933, 14361, 12298, 12296,
	2914, 14881, 15072, 1412, 10685, 10017, 10016, 1420, 14681, 16337,
	15071, 12152, 1, 3681, 12861
};
//-----------
...
//L10 MODIFIED
//New method
bool IsCritter(uint32 entry) {
	return (std::find(critterEntries.begin(), critterEntries.end(), entry) != critterEntries.end());
}
...
void Spell::cast(bool skipCheck)
{
	...
	//L10 MODIFIED
	//Adding player spell school skills calculations -------------------
	if (m_caster->IsPlayer()) {
		//Chance for the spell cast to fail... only skillable spells should fail
		if (m_caster->ToPlayer()->ShouldSpellFail(spellInfo)) {
			if (roll_chance_i(20)) {
				//Self harm
				m_caster->CastSpell(m_caster->ToUnit(), m_caster->ToPlayer()->GetSelfHarmSpell(spellInfo->School), true);
			}

			//This one got broken after some vmangos core update, now it shoots the fireball out
			//When all this did was cast the shoot hand animation not the spell animation itself
			//SendSpellGo();
			...
			if (spellInfo->CastingTimeIndex == 0) {
				m_caster->InterruptNonMeleeSpells(false, spellInfo->Id);
			}
			...
			if (spellInfo->GetCastTime() == 0 && m_caster->ToPlayer()->GetObjectGuid()) {
				m_caster->ToPlayer()->SendSpellCooldown(spellInfo->Id, 1, m_caster->ToPlayer()->GetObjectGuid());
			}
		...
		else if (!m_caster->ToPlayer()->IsSpellRejected(spellInfo)) {
			//Check if only critters were hit
			bool onlyCritters = true;
			if (m_UniqueTargetInfo.size() >= 1) {
				for (auto& itr : m_UniqueTargetInfo) {
					if (itr.targetGUID) {
						if (!IsCritter(itr.targetGUID.GetEntry())) {
							onlyCritters = false;
							break;
						}
					}
			...
			if(!onlyCritters || m_UniqueTargetInfo.size() == 0)
				m_caster->ToPlayer()->UpdateSpellSkill(spellInfo, m_targets.getUnitTarget(), m_UniqueTargetInfo.size());
		}
	}
	//------------------------------------------------------------------
