...
function PaperDollFrame_SetStats()
	for i=1, NUM_STATS, 1 do
		...
		if ( ( posBuff == 0 ) and ( negBuff == 0 ) ) then
			...
			-- L10 MODIFIED
			-- Lets make it yellow if both negative and positive effects are in place
			if(negBuff < 0 and posBuff > 0) then
				text:SetText(NORMAL_FONT_COLOR_CODE..effectiveStat..FONT_COLOR_CODE_CLOSE);
...
function PaperDollFrame_SetResistances()
	for i=1, NUM_RESISTANCE_TYPES, 1 do
		...
		-- L10 MODIFIED
		unitLevel = max(unitLevel, 3);
		local magicResistanceNumber = resistance/unitLevel*6;
		-- --
...
function PaperDollFrame_SetArmor(unit, prefix)
	...
	-- //L10 MODIFIED
	local armorReduction = effectiveArmor/((85 * (playerLevel * 6)) + 400);
