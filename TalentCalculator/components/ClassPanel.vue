<template>
  <div :class="['talent-trees', { 'is-max-level' : this.classType.availableSkillPoints == 0 }]">
    <talent-tree
...
export default {
  ...
  methods: {
    ...
    increaseRequiredLevel: function() {
      ...
      else if((50 - (this.classType.availableSkillPoints + 1)) % 5 == 0) this.classType.requiredLevel++;

      this.checkMaxLevel();
    },
    decreaseRequiredLevel: function(points) {
      if(points == 1) this.classType.requiredLevel = Math.ceil((51 - (this.classType.availableSkillPoints + points)) / 5);
      else this.classType.requiredLevel = Math.abs(Math.ceil(((this.classType.availableSkillPoints) - 50) / 5));
